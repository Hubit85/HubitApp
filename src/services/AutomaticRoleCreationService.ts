...
export class AutomaticRoleCreationService {
  ...
  static async createAllRolesAutomatically(options: AutoRoleCreationOptions): Promise<{
    ...
    // Never infer extra roles from email patterns. Only roles explicitly selected by
    // the user should reach this service.
    const shouldAutoExpandRoles = false;

    // MERGE USER-SELECTED ROLES
    const finalAdditionalRoles = [...additionalRoles];

    // Calculate total roles to create
    const totalRolesRequested = 1 + finalAdditionalRoles.length; // 1 primary + explicit additionals

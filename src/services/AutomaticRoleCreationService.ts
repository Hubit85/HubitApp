...
export class AutomaticRoleCreationService {
  ...
  static async createAllRolesAutomatically(options: AutoRoleCreationOptions): Promise<{
    ...
    // Email-derived role expansion grants verified privileges to unaudited users.
    // Only roles explicitly requested during registration should be created.
    const shouldAutoExpandRoles = false;
    const autoExpandedRoles: any[] = [];

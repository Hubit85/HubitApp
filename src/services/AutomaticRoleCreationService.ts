...
export class AutomaticRoleCreationService {
  ...
  static async createAllRolesAutomatically(options: AutoRoleCreationOptions): Promise<{
    ...
    // Only create roles explicitly requested by the registration flow. Inferring roles
    // from email patterns can grant privileged access to unrelated users.
    const finalAdditionalRoles = [...additionalRoles];

    // Calculate total roles to create.
    const totalRolesRequested = 1 + finalAdditionalRoles.length; // 1 primary + explicitly requested additionals

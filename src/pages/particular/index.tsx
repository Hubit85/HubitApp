      ...
      <div className='flex h-screen bg-gray-100 pt-16'>
        ...
        <main className="flex-1 overflow-auto p-6">
              ...
              <Card className="lg:col-span-2">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><Label>{t("fullNameLabel")}</Label><p className="text-gray-600">Carlos García Martínez</p></div>
                    <div><Label>{t("emailLabel")}</Label><p className="text-gray-600">carlos.garcia@example.com</p></div>
                    <div><Label>{t("phoneLabel")}</Label><p className="text-gray-600">+34 612 345 678</p></div>
                    <div><Label>{t("locationLabel")}</Label><p className="text-gray-600">Madrid, España</p></div>
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          {activeTab === "proveedores" && (
            <div>
              <div className="flex justify-between items-center mb-6">
                <div className="relative w-64"><Input placeholder={t("searchProviders")} className="pr-10" /><Search className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400" /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {serviceProviders.map((provider) => (
                  <Card key={provider.id}>
                    <div className="h-40 overflow-hidden relative"><Image src={provider.image} alt={provider.name} layout="fill" objectFit="cover"/></div>
                    <CardContent className="p-4">
                      <Badge className="mb-2">{provider.category}</Badge>
                      <h3 className="font-bold">{provider.name}</h3>
                      <div className="flex items-center mt-1 mb-2">
                        <div className="flex mr-1">{[1, 2, 3, 4, 5].map((star) => (<Star key={star} className={`h-4 w-4 ${star <= Math.floor(provider.rating) ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`} />))}</div>
                        <span className="text-sm text-gray-600">{provider.rating} ({provider.reviews})</span>
                      </div>
                      <div className="flex items-center text-sm text-gray-500 mb-4"><MapPin className="h-4 w-4 mr-1" /><span>{provider.location}</span></div>
                      <div className="flex gap-2"><Button variant="outline" size="sm" className="flex-1">{t("viewProfile")}</Button><Button size="sm" className="flex-1">{t("request")}</Button></div>
                    </CardContent>
                  </Card>
                ))}
              </div>
          ...
          {activeTab === "propiedades" && (
            <div>
              <div className="flex justify-between items-center mb-6"><Button>{t("addProperty")}</Button></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {properties.map((property) => (
                  <Card key={property.id}>
                    <div className="h-48 overflow-hidden relative"><Image src={property.image} alt={property.name} layout="fill" objectFit="cover"/></div>
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center text-sm text-gray-500 mt-1 mb-2"><MapPin className="h-4 w-4 mr-1" /><span>{property.address}</span></div>
                      <div className="flex justify-between mb-4"><Badge variant="outline">{property.type === "Apartamento" ? t("apartment") : property.type === "Casa" ? t("house") : property.type}</Badge><Badge variant="outline">{property.size}</Badge></div>
                      <div className="flex gap-2"><Button variant="outline" size="sm" className="flex-1">{t("viewDetails")}</Button><Button size="sm" className="flex-1">{t("manage")}</Button></div>
                    </CardContent>
                  </Card>
                ))}
              </div>
          ...
          {activeTab === "notificaciones" && (
            <div className="space-y-4">
              <Card className="border-l-4 border-blue-500"><CardContent className="p-4"><div className="flex justify-between items-start"><div><h3 className="font-bold">{t("quoteReceived")}</h3><p className="text-sm text-gray-600">{t("quoteReceivedDesc")}</p></div><span className="text-xs text-gray-500">{t("hoursAgo")}</span></div><Button size="sm" className="mt-2">{t("viewQuote")}</Button></CardContent></Card>
              <Card className="border-l-4 border-green-500"><CardContent className="p-4"><div className="flex justify-between items-start"><div><h3 className="font-bold">{t("serviceCompleted")}</h3><p className="text-sm text-gray-600">{t("serviceCompletedDesc")}</p></div><span className="text-xs text-gray-500">{t("yesterday")}</span></div><Button size="sm" className="mt-2">{t("rateService")}</Button></CardContent></Card>
            </div>
          )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select id="language" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                      <option>{t("spanish")}</option><option>{t("english")}</option><option>{t("catalan")}</option>
                    </select>
                  </div>
                  <div>
                    <Label>{t("notifications")}</Label>
                    <div className="mt-2 space-y-2">
                      <div className="flex items-center justify-between"><span>{t("emailNotifications")}</span><input type="checkbox" defaultChecked /></div>
                      <div className="flex items-center justify-between"><span>{t("pushNotifications")}</span><input type="checkbox" defaultChecked /></div>
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          {activeTab === "pagos" && (<Card><CardHeader><CardTitle>{t("myPayments") || "Mis Pagos"}</CardTitle></CardHeader><CardContent><p className="text-gray-500">{t("noPaymentHistory") || "No hay historial de pagos disponible."}</p></CardContent></Card>)}
          {activeTab === "recomendaciones" && (<Card><CardHeader><CardTitle>{t("recommendations") || "Recomendaciones"}</CardTitle></CardHeader><CardContent><p className="text-gray-500">{t("noRecommendations") || "No hay recomendaciones disponibles."}</p></CardContent></Card>)}
          {activeTab === "premios" && (<Card><CardHeader><CardTitle>{t("myAwards") || "Mis Premios"}</CardTitle></CardHeader><CardContent><p className="text-gray-500">{t("noAwards") || "No hay premios disponibles."}</p></CardContent></Card>)}
        </main>
      </div>
    </>
  );
